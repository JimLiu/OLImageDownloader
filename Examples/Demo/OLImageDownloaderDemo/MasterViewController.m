...
//
//  Created by junmin liu on 13-3-18.
...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self) {
        _downloader = [OLImageDownloader downloaderWithName:@"photos"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    __strong NSString *imageUrl = [_objects objectAtIndex:indexPath.row];
    cell.textLabel.text = imageUrl;
    [cell.imageView setImage:[UIImage imageNamed:@"placeholder_image.png"]];
    __weak UITableViewCell *wcell = cell;
    [_downloader downloadImageWithURL:[NSURL URLWithString:imageUrl]
                                                               completed:^(UIImage *image, NSData *data, NSError *error, BOOL finished)
    {
        ...
        if (image && finished
            && [imageUrl isEqualToString:sCell.textLabel.text]
            )
        {
            sCell.imageView.image = image;
            [sCell.imageView setNeedsLayout];
        }
